      ...
      <div className="flex flex-row items-center  gap-2">
        ...
        <div className="flex  h-full items-center gap-2">
          {items.map((item: string, index: number) => (
            <div className="">
              <Pill key={index} withRemoveButton onRemove={() => removeFilter(label, item)} className="2xl:text-md bg-[#D9D9D9] text-[#6D6D6D] font-semibold">
                {item}
              </Pill>
            </div>
          ))}
    ...
    return (
      <div className="flex flex-row items-center  gap-2 ">
        ...
        <div className="flex h-full items-center">
          <div className="">
            <Pill withRemoveButton onRemove={() => removeFilter(label, item)} className="2xl:text-md bg-[#D9D9D9] text-[#6D6D6D] font-semibold">
              {item}
            </Pill>
          </div>
        </div>
      ...
      <div className="flex flex-row items-center  gap-2 ">
        ...
        <div className="flex h-full items-center">
          <div className="flex gap-1">
            ...
            -
            <Pill withRemoveButton onRemove={() => removeFilter(label2, item2)} className="2xl:text-md bg-[#D9D9D9] text-[#6D6D6D] font-semibold">
              {formatDate(item2)}
            </Pill>
          </div>
        </div>
    ...
    <div className="w-full rounded-lg  flex flex-row justify-between items-center bg-white h-full border-2 border-black-600">
      ...
      {isFiltered && (
        <div className="scrollbar flex flex-wrap h-full w-full overflow-hidden px-4 gap-2 sm:overflow-x-hidden sm:hover:overflow-y-auto p-1">
          ...
          {filter.dateFrom && !filter.dateTo && renderSinglePill("DateFrom", filter.dateFrom)}

          {filter.dateTo && !filter.dateFrom && renderSinglePill("DateTo", filter.dateTo)}

          {filter.dateTo && filter.dateFrom && renderDateRange("dateFrom", filter.dateFrom, "dateTo", filter.dateTo)}

          {filter.vacancy && filter.vacancy.length > 0 && renderPills("Vacancy", filter.vacancy)}

          {filter.interviewer && filter.interviewer.length > 0 && renderPills("Interviewer", filter.interviewer)}

          {filter.department && filter.department.length > 0 && renderPills("Department", filter.department)}
      ...
      <div className="flex h-full items-center px-3">
        ...
        <ActionIcon onClick={() => setClearFilter(true)} variant="transparent" color="gray" size={iconSize} aria-label="Settings">
