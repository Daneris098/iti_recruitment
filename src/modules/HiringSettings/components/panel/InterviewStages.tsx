...
const InterviewStage = forwardRef((_, ref) => {
  const { interviewStage, setInterviewStage, setSortStatus, sortStatus } = InteviewStagesStore();
  ...
  const columns: any = [
    {
      ...
      title: (
        <p>
          Sequence No <span className={operation === Operation.noOperation ? `hidden` : `text-red-500 `}>*</span>
        </p>
      ),
      sortable: true,
      render: (data: any) =>
        interviewStagesEditMode[data.id] && data.fieldStatus === "new" ? (
          <NumberInput readOnly value={interviewStagesEditableData[data.id]?.sequenceNo || data.sequenceNo} onChange={(e: any) => handleEditChange(data.id, "sequenceNo", e)} />
        ) : (
          <p>{data.sequenceNo}</p>
        ),
    },
    {
      ...
      title: (
        <p>
          Stage Name <span className={operation === Operation.noOperation ? `hidden` : `text-red-500 `}>*</span>
        </p>
      ),
      sortable: true,
      render: (data: any) =>
        interviewStagesEditMode[data.id] && data.fieldStatus === "new" ? (
          <TextInput
            ...
            error={interviewStagesEditableData[data.id]?.stageName === "" ? "Stage name is Required" : null}
    ...
    },
    {
      ...
      title: (
        <p>
          Status <span className={operation === Operation.noOperation ? `hidden` : `text-red-500 `}>*</span>
        </p>
      ),
      sortable: true,
      render: (data: any) =>
  ...
  const fetchData = async () => {
    let url = "/recruitment/hiring/interview-stages";
  ...
  const handleEditChange = (id: number, field: keyof interviewStage, value: string | number) => {
  ...
  const toggleEditMode = (id: number) => {
    ...
    if (!interviewStagesEditMode[id]) {
      const rowData = interviewStage.find((item) => item.id === id) || interviewStagesNewRows.find((item) => item.id === id);
    ...
    <div className="flex flex-col gap-8 h-[100%]">
      <div className="flex flex-col gap-2 ">
        <div className="flex gap-2">
          ...
          <div>
            <IconCirclePlus style={{ height: "100%" }} color="#5A9D27" onClick={addNewRow} />
          </div>
        </div>
      ...
      <DataTable
        ...
        withTableBorder
        records={[...interviewStagesNewRows, ...interviewStage]}
        columns={columns}
