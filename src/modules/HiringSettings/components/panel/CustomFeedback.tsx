...
import { IconCirclePlus, IconPencil, IconTrashFilled, IconCircleX, IconArrowsMoveVertical } from "@tabler/icons-react";
...
const CustomFeedback = forwardRef((_, ref) => {
  ...
  const { applicantFeedback, setApplicantFeedback, hiringFeedback, setHiringFeedback, sortStatusApplicant, sortStatusHiring, setSortStatusApplicant, setSortStatusHiring } =
    FeedbackStore();
  ...
  const toggleEditMode = (id: number, mode: FeedbackMode) => {
    ...
    if (mode === "applicantFeedback") {
      ...
      if (!applicantEditMode[id]) {
        const rowData = applicantFeedback.find((item) => item.id === id) || applicantNewRows.find((item) => item.id === id);
      ...
      if (!hiringEditMode[id]) {
        const rowData = hiringFeedback.find((item) => item.id === id) || hiringNewRows.find((item) => item.id === id);
  ...
  const handleEditChange = (id: number, field: keyof feedback, value: string, mode: string) => {
    if (mode === "applicantFeedback") {
  ...
  const saveAll = () => {
    ...
    if (operation == Operation.add) {
      ...
      if (applicantEditableData && Object.keys(applicantEditableData).length > 0) {
        updateFeedback({
          id: Object.values(applicantEditableData)[0]?.id,
          description: Object.values(applicantEditableData)[0]?.feedback,
          isApplicantFeedback: true,
          guid: Object.values(applicantEditableData)[0]?.guid,
        });
      } else {
        updateFeedback({
          id: Object.values(hiringEditableData)[0]?.id,
          description: Object.values(hiringEditableData)[0]?.feedback,
          isApplicantFeedback: false,
          guid: Object.values(hiringEditableData)[0]?.guid,
        });
      }
    }
    ...
    applicantFeedback: [
      {
        ...
        title: (
          <div className="flex justify-between">
            <p>
              Applicant Feedback <span className={operation != Operation.noOperation && applicantNewRows.length > 0 ? `text-red-500` : `hidden`}>*</span>
            </p>
        ...
        sortable: false,
        render: (data: any) =>
          applicantEditMode[data.id] && data.fieldStatus === "new" ? (
            <TextInput
              className="w-full"
    ...
    hiringFeedback: [
      {
        ...
        title: (
          <div className="flex justify-between">
            <p>
              Hiring Feedback <span className={operation != Operation.noOperation && hiringNewRows.length > 0 ? `text-red-500 ` : `hidden`}>*</span>
            </p>
            <div className="flex gap-2 items-center">
              <IconArrowsMoveVertical
                size={15}
        ...
        sortable: false,
        render: (data: any) =>
          hiringEditMode[data.id] && data.fieldStatus === "new" ? (
            <TextInput
  ...
  const checkEditIsValid = (operationFrom?: string) => {
    ...
    return (
      !Object.entries(applicantEditableData).some(([_, data]) =>
  ...
  const rowExpansion1: any = {
    ...
    expandable: ({ record: { isNewField } }: any) => {
      return !isNewField;
    },
    ...
    <div className="flex flex-col h-[100%] gap-8  ">
      <div className="flex flex-col mb-4 gap-2">
        ...
        <p className="text-[#6D6D6D]">
          This section allows you to add custom feedback from both the hiring team (after every interview) and the applicant (upon receiving a job offer). If no feedback is
          provided by the applicant, select "No Response."
        </p>
      </div>
      <div className="flex justify-start gap-[15%] h-[78%]">
        <div className="w-[30%]">
          <DataTable
            ...
            rowClassName={(row: any) => (row.id === selectedRowId ? "bg-[#DEECFF]" : "")}
            withTableBorder
        ...
        <div className="w-[30%] ">
          <DataTable
            ...
            rowClassName={(row: any) => (row.id === selectedRowId2 ? "bg-[#DEECFF]" : "")}
            withTableBorder
