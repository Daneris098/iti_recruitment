...
const Interviewers = forwardRef((_, ref) => {
  ...
  const columns: any = [
    {
      ...
      render: (data: any) =>
        interviewStagesEditMode[data.id] && data.fieldStatus === "new" ? (
          <TextInput value={interviewStagesEditableData[data.id]?.name || data.name} onChange={(e: any) => handleEditChange(data.id, "name", e.target.value)} />
        ) : (
          <p>{data.name}</p>
        ),
    },
  ...
  const handleEditChange = (id: number, field: keyof interviewer, value: string) => {
  ...
  const toggleEditMode = (id: number) => {
    ...
    if (!interviewStagesEditMode[id]) {
      const rowData = interviewers.find((item) => item.id === id) || interviewStagesNewRows.find((item) => item.id === id);
      ...
      return (
        <div className=" flex gap-2 relative  ">
          ...
          <TextInput
            ...
            classNames={{ input: "poppins text-[#6D6D6D]" }}
            styles={{ label: { color: "#6d6d6d" } }}
            value={formatDate(String(interviewStagesEditableData[id]?.lastModified)) || lastModified}
          />
          <Select
            radius={8}
            data={["ACTIVE", "INACTIVE"]}
    ...
    <div className="flex flex-col gap-8 h-[100%]">
      <div className="flex flex-col gap-2">
        <div className="flex gap-2">
          ...
          <div>
            <IconCirclePlus className="cursor-pointer" style={{ height: "100%" }} color="#5A9D27" onClick={addNewRow} />
          </div>
        </div>
