  ...
  const onSubmit = async (form: any = { reApplyInMonths: 1, allowReApply: true, offerResponsePeriodInDays: 3 }) => {
    ...
    try {
      if (otherSettingsId.id != "") {
        ...
        await axiosInstance.post(`/recruitment/hiring/other-settings/${otherSettingsId.id}/update`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
      } else {
        await axiosInstance.post("/recruitment/hiring/other-settings", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
      ...
      <div className="flex flex-col gap-6 ">
        <div className="flex flex-col gap-8 border-[2px] border-blue-300 rounded-md px-4 sm:h-[90%] p-4 sm:p-8">
          ...
          <div className="flex w-[70%] justify-between">
            ...
            <NumberInput
              ...
              hideControls
              min={1}
              disabled={form.getValues().allowReApply === "Yes" ? false : true}
              max={12}
        ...
        <div className="flex flex-col gap-8 border-[2px] border-blue-300 rounded-md px-4 sm:h-[90%] p-4 sm:p-8">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-[#6D6D6D]">
              Establish the maximum number of days for applicants to sign the job offers issued. Once this limit is reached, the job offer will be automatically marked as declined,
              and the applicant's status will be updated to Archived."
            </p>
          </div>
          <NumberInput
