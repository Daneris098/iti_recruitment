...
import {
  ...
  DEFAULT_PAGE_NUMBER,
  LABEL_ARCHIVED,
  BASE_COLUMNS_WITH_STATUS,
  COMPUTED_SECONDS,
  HEADER_GENERATE_JOB_OFFER,
  BASE_COLUMNS_WITH_REMARKS,
  SHOWING_TOTAL_DATA,
...
export default function index() {
  ...
  if (filter.applicantName) {
    queryParams.Name = filter.applicantName;
  }
  if (selectedStatusId) {
    queryParams.StatusIds = selectedStatusId;
  }
  ...
  const { data: sharedApplicants } = useApplicants(page, pageSize, queryParams, setLoadTime);

  const hiredApplicantIds = useMemo(() => {
    return sharedApplicants?.applicants?.filter((a) => a.status === STATUS_HIRED)?.map((a) => a.id) || [];
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      const offers = results.filter((result) => result.offer !== null).map((result) => result.offer);
  ...
  const renderCell = (col: { accessor: string }, row: Row) => {
    ...
    return <span className={`flex items-center ${col.accessor === STATUS ? "flex items-center justify-center" : ""} ${cursorClass}`}>{content}</span>;
  };
  ...
  const enhancedColumns = getFilteredColumns(activeTab!).map((col) => ({
    ...col,
    title: col.sortable ? <span onClick={() => setSort(col.accessor, records)}>{col.title}</span> : col.title,
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <Tabs value={activeTab} onChange={(value) => setActiveTab(value as TabKey)} defaultValue={OFFERS_TAB} className="flex flex-col h-full">
              ...
              <Tabs.Panel key={tab.value} value={tab.value} className="h-full flex flex-col">
                <div className="flex-grow overflow-auto">
                  <DataTable
                    ...
                    onSortStatusChange={({ columnAccessor, direction }) => setSort(String(columnAccessor), records, direction)}
          ...
          <div className="flex-shrink-0 mt-4 pt-2 border-t flex justify-between items-center text-[#6D6D6D]">
            <p className="job-offers-table text-sm poppins">
              {`${SHOWING_TOTAL_DATA} ${(page - 1) * pageSize + 1} ${TO} ${Math.min(page * pageSize, filteredRecords.length)} ${OF} ${filteredRecords.length} ${TOTAL_ENTRIES}`}
              {loadTime !== null && ` ${FOUND_IN} (${loadTime.toFixed(3)}) ${COMPUTED_SECONDS}`}
            </p>
            <Pagination value={page} onChange={handlePageChange} total={Math.ceil(filteredRecords.length / pageSize)} siblings={1} size="sm" />
          </div>
        </Tabs>
      </div>
      ...
      <PDFModal isOpen={!!selectedRow} onClose={() => setSelectedRow(null)} header={HEADER_GENERATE_JOB_OFFER}>
        {selectedRow && (
          <iframe
            src={`${import.meta.env.VITE_REPORTS_BASE_URL}/Report/Get/?filter=ReportFilename=HRDotNet_Recruitment_Job_Offer_v1@ID_Applicant=${applicantId}`}
            title="PDF Viewer"
            width="100%"
            height="1000px"
            style={{ border: "none" }}
