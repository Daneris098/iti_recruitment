            ...
            {(notifications).map((item: Notification, index: number) => (
                <>
                    <Container
                        key={index}
                        py={10}
                        className={`gap-10 w-full cursor-pointer select-none ${item.notif_status === 0 && "bg-blue-100"}`}
                    >
                        <Flex align="start" justify="start" gap={10}>
                            <Avatar variant="filled" radius="md" color="rgb(219, 234, 254)" size={30}>
                                {item.filing_status === "application" && (<IconUserPlus
                                    color="#559CDA"
                                    style={{ width: "100%", height: "100%" }}
                                    stroke={1}
                                />)}
                                {item.filing_status === "events" && (<IconPhoneCall
                                    color="#ED8028"
                                    style={{ width: "100%", height: "100%", background: "#FFD8B6" }}
                                    stroke={1}
                                />)}
                                {item.filing_status === "actions" && (<IconChecklist
